...
func _write_input_ui():
	...
	var str = format % [Controls.movement_input, Controls.move_dir.normalized(), round(Controls.move_dir.length()), round(Controls.move_dir.length() * 39.37), Controls.jump_on, false]
